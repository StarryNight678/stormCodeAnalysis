...
注意,搜集Metric将导致,整体吞吐量降低.
One of idea to avoid this is making your implementation of Metrics Consumer as non-blocking fashion.
...
```java
 46 ## Metrics Consumers
 47 #default
 48 #topology.stats.sample.rate: 0.05
 49  topology.stats.sample.rate: 1
 50  topology.builtin.metrics.bucket.size.secs: 5
 51  topology.metrics.consumer.register:
 52    - class: "org.apache.storm.metric.LoggingMetricsConsumer"
 53      parallelism.hint: 1
```


- Class IMetricsConsumer.TaskInfo


```java
String  srcComponentId 
int srcTaskId 
String  srcWorkerHost 
int srcWorkerPort 
long    timestamp 
...
1. capacity需要小于1 look at the capacity of the decode_bytes_1 and save_to_cassandra spouts. Both are over 1 (the sum of all spouts capacity should be under 1), which means you are using more resources than what do you have available. This is, the topology can't handle the load.

1. 注意修改TOPOLOGY_MAX_SPOUT_PENDING The TOPOLOGY_MAX_SPOUT_PENDING will solve your problem if the throughput of tuples varies during the day. This is, if you have peek hours, and you will be catch up during the off-peek hours.

1. You need to increase the number of worker machines or optimize the code in the bottle neck spouts (or maybe both). Otherwise you will not be able to process all the tuples.

1. You probably can improve the cassandra persister by inserting in batches instread of insert tuples one by one...

1. I seriously recommend you to always set the TOPOLOGY_MAX_SPOUT_PENDING for a conservative value. The max spout pending, means the maximum number of un-acked tuples inside the topology, remember this value is multiplied by the number of spots and the tuples will timeout (fail) if they are not acknowledged 30 seconds after being emitted.

1. And yes, your problem is having tuples timing out, this is exactly what is happening.

1. (EDIT) if you are running the dev environment (or just after deploy the topology) you might experience a spike in the traffic generated by messages that were not yet consumed by the spout; it's important you prevent this case to negatively affect your topology -- you never know when you need to restart the production topology, or perform some maintenance --, if this is the case you can handle it as a temporary spike in the traffic --the spout needs to consume all the messages produced while the topology was off-line -- and after a some (or many minutes) the frequency of incoming tuples stabilizes; you can handle this with max pout pending parameter (read item 2 again).

1. Considering you have 3 nodes in your cluster, and cpu usage of 0,1 you can add more executers to the bolts.




## [Nimbus.Client](http://storm.apache.org/releases/1.0.3/javadocs/org/apache/storm/generated/Nimbus.Client.html)
...
```java
double  get_assigned_cpu() 
double  get_assigned_memonheap() 
String  get_status() 
topologyInfo.get_requested_cpu()
...
```java
double  get_assigned_cpu() 
double  get_assigned_memoffheap() 
double  get_assigned_memonheap() 
String  get_sched_status() 
